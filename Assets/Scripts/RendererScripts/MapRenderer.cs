...
public class MapRenderer : SpriteLoader
{
    ...
    private void updateSortingLayerValue()
    {
        ...
        // NOTE: this child ("GameObject" in Home Tile.prefab) is the floor/ground
        // sprite under the house, on the "Ground" sorting layer - NOT the house
        // itself. The house is the ROOT sprite (this component, "Objects" layer),
        // and it's already assigned a correct sortingOrder by the loop above, same
        // reference frame as every other tile. The actual "tower renders in front
        // of the house" bug was towers being sorted by an independent counter
        // uncalibrated against tiles at all - fixed in TurretSpriteRenderer.cs,
        // which now reads each tower's order directly off its own tile (see
        // PlacementManager.GetTileOfTower). This line only keeps the floor's order
        // in sync with its own tile for tidiness; being on a different layer, its
        // exact value has no bearing on house-vs-tower depth.
