...
public class MapGenerator : MonoBehaviour {
    ...
    /// <summary>
    /// Half the tile's largest footprint dimension. Used as a "close enough" cutoff for
    /// distance-based hover picking (see PlacementManager.GetCurrentHoverTile) - lets it
    /// tell "cursor is over some tile" from "cursor is off in empty space past the edge
    /// of the map" without needing per-tile colliders.
    /// </summary>
    public float getTileHoverRadius() {
        return Mathf.Max(spriteWidth, spriteHeight) * 0.5f;
    }
    ...
    public void randomExpand() {
        if (expandableTiles.Count == 0) {
            // Genuine dead end - no tileset arrangement left that can extend the path
            // further. Stop expanding cleanly instead of indexing an empty list.
            Debug.Log("No expandable tilesets - map generation stopped for this match.");
            return;
        }
    ...
    public bool expandMap(MapLayout locTileInfo) {   // expands the map
        if (expandableTiles.Count == 0) {
        ...
        if (availableDirections.Count == 0) {
            // Every direction here is already occupied by existing map geometry.
            // Placing a chunk with no constraint (the old behavior) would overlap
            // previously-placed tiles - that's the path looping back into itself.
            // Drop this candidate and bail instead of generating a colliding chunk.
            Debug.Log("Expansion point is fully boxed in - skipping to avoid overlapping the existing path.");
            expandableTiles.RemoveAll(x => x.position == locTileInfo.position);
            return false;
        }
        ...
        if (availableDirections.Count == 3) {
    ...
    private void generateMap() {   // generates the initial map
        MapRenderer.triggerRenderer();
