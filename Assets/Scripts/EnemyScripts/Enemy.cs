...
public class Enemy : MonoBehaviour {
    ...
    protected bool enemyFinished = false;

    // "Kiểm tra HP và hướng đi của Enemy: ... highlight lối đi của Enemy." - which
    // entry in MapGenerator.pathTiles this enemy is walking, so a hover-UI can
    // highlight just this enemy's route instead of every path on the map.
    ...
    protected virtual void enemyDead() {
        ...
        if (enemyHealth <= 0 && WoodManager.main != null)
            WoodManager.main.AddWood(killReward);

        Counter.enemies.Remove(gameObject);

        // Adapted from the uploaded RoundController.cs's design: the enemy that
        // empties Counter.enemies calls the wave-complete transition directly,
        // instead of only waiting for RoundController's own FixedUpdate() poll to
        // notice next tick. OnRoundComplete() is guarded (isRoundGoing) so this
        // can't double-award alongside that poll.
        if (Counter.enemies.Count == 0 && RoundController.main != null)
            RoundController.main.OnRoundComplete();
