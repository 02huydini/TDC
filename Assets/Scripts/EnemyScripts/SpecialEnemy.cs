...
public class SpecialEnemy : Enemy
{
    ...
    protected override void enemyDead()
    {
        ...
        if (enemyHealth <= 0)
        {
            ...
            if (WoodManager.main != null) WoodManager.main.AddWood(killReward);
        }

        Counter.enemies.Remove(gameObject);

        // See Enemy.cs's enemyDead() for why this call is safe alongside
        // RoundController's own FixedUpdate() poll (OnRoundComplete() is guarded).
